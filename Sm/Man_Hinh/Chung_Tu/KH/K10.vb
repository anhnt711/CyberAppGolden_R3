...
#Region "Columns Edit"
    ...
    Dim EditMa_Kx, EditTen_kx, EditTen_LapKH_I, EditMa_Mau, EditMa_Mau_Nt, EditTen_Mau As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    ...
    Private Sub K10_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        CreateTableMonth()
        V_Load()
        V_GetColumn()
        V_AddHandler()
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub L_Ma_Mau(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim drReturn As DataRow
        iRows = DetailGRV.GetFocusedDataSourceRowIndex
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not ChkDa_KH(M_Mode) Then Exit Sub
    ...
    End Sub
    Private Function ChkDa_KH(ByVal _Mode) As Boolean
        Dim _Dt As Date = TxtNgay_Ct.Value
        Dim nCount As Integer
        nCount = Dt_DetailTmp.Rows.Count
        If nCount = 0 Then
            CyberSupport.V_MsgChk("N", "Y", "Không có số liệu chi tiết bạn không lưu được", osysvar)
            Return False
        End If
        nCount = Dt_DetailTmp.Rows.Count
        ...
        Dim strSo_Hang As String = ""
        Dim strSo_Khung As String = ""
        Dim strSo_May As String = ""
        Dim _StrDetail As String = ""

        CyberSmodb.SetNotNullTable(Dt_DetailTmp)

        Dim _row As Double = Dt_DetailTmp.Rows.Count
        Dim _ThangKh As String = CbbThang.SelectedValue
        Dim _NamKh As String = CbbNam.SelectedValue
        '-----ThangTv---------------------------------------

        For i = 0 To Dt_DetailTmp.Rows.Count - 1
            _StrDetail = _StrDetail & ";INSERT KeHoach10 SELECT "
            _StrDetail = _StrDetail & " N'" & Dt_DetailTmp.Rows(i).Item("MA_LAPKH_I").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & Dt_DetailTmp.Rows(i).Item("Ma_mau").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & Dt_DetailTmp.Rows(i).Item("So_Hang").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & Dt_DetailTmp.Rows(i).Item("So_khung").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & Dt_DetailTmp.Rows(i).Item("So_may").ToString.Trim & "'"
        Next
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value

        DsTmp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkK10", _StrDetail & "#" & _Mode & "#" & _ThangKh & "#" & _NamKh & "#" & M_Stt_Rec & "#" & TxtMa_Dvcs.Text & "#" & M_User_Name)
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            Dt_DetailTmp.Rows(iRow).Item("Ma_kx") = Dt_DetailTmp.Rows(iRow).Item("Ma_kx")
            ...
            If Dt_DetailTmp.Columns.Contains("Ma_KH_I") Then If TxtMa_KH.Text.Trim <> "" Then Dt_DetailTmp.Rows(iRow).Item("Ma_KH_I") = TxtMa_KH.Text.Trim
        Next
        Dt_DetailTmp.AcceptChanges()
        ...
        ChkVsave = True
    End Function
    Private Function Add2DatabaseLuu2() As Boolean
        '-----------------------------------------------------------------------------------------------------
    ...
    Private Sub L_Ma_mau_Nt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            ...
            If Dv_DetailTmp.Table.Columns.Contains("Ten_Mau_Nt") Then Dv_DetailTmp.Item(iRow).Item("Ten_Mau") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
...
#Region "Add Or Detete Item"
    Private Sub MasterGRV_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        Dim iRowRowFocus As Integer = DetailGRV.GetFocusedDataSourceRowIndex
...
#Region "Sms - Import"
    Private Sub V_Import(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _StrDetail As String = ""
        Dim _row As Double = TbImport.Rows.Count
        Dim ngay_gx_str As String = ""
        Dim ngay_gx As Date = Date.Parse("1900-01-01")
        Dim ngay_tt_str As String = ""
        Dim ngay_tt As Date = Date.Parse("1900-01-01")
        Dim ngay_dm_str As String = ""
        Dim ngay_dm As Date = Date.Parse("1900-01-01")
        Dim ngay_sx_str As String = ""
        Dim ngay_sx As Date = Date.Parse("1900-01-01")
        Dim Invoid_Date_str As String = ""
        Dim Invoid_Date As Date = Date.Parse("1900-01-01")
        For i = 0 To TbImport.Rows.Count - 1
            ngay_gx_str = TbImport.Rows(i).Item("Ngay_GX").ToString.Trim
            If Date.TryParse(ngay_gx_str, ngay_gx) = False Then
                ngay_gx = Date.Parse("1900-01-01")
            End If
            ngay_tt_str = TbImport.Rows(i).Item("Ngay_TT").ToString.Trim()
            If Date.TryParse(ngay_tt_str, ngay_tt) = False Then
                ngay_tt = Date.Parse("1900-01-01")
            End If
            ngay_dm_str = TbImport.Rows(i).Item("Ngay_DoiMau").ToString.Trim()
            If Date.TryParse(ngay_dm_str, ngay_dm) = False Then
                ngay_dm = Date.Parse("1900-01-01")
            End If
            ngay_sx_str = TbImport.Rows(i).Item("Ngay_SX").ToString.Trim()
            If Date.TryParse(ngay_sx_str, ngay_sx) = False Then
                ngay_sx = Date.Parse("1900-01-01")
            End If
            Invoid_Date_str = TbImport.Rows(i).Item("Invoid_Date").ToString.Trim()
            If Date.TryParse(Invoid_Date_str, Invoid_Date) = False Then
                Invoid_Date = Date.Parse("1900-01-01")
            End If
            _StrDetail = _StrDetail & ";INSERT ImpK10 SELECT "
            _StrDetail = _StrDetail & " N'" & TbImport.Rows(i).Item("MA_LAPKH_I").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Ten_LAPKH_I").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Ma_mau").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Ten_mau").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Ma_mau_Nt").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("So_Hang").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("So_khung").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("So_may").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Dien_giai").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Gia_nt").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & ngay_gx.ToString("yyyyMMdd") & "'"
            _StrDetail = _StrDetail & ", N'" & ngay_tt.ToString("yyyyMMdd") & "'"
            _StrDetail = _StrDetail & ", N'" & ngay_dm.ToString("yyyyMMdd") & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Kho_bai").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Van_chuyen").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Xe_CuuHo").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & ngay_sx.ToString("yyyyMMdd") & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Nam_Sx").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & Invoid_Date.ToString("yyyyMMdd") & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("Invoid_No").ToString.Trim & "'"
            _StrDetail = _StrDetail & ", N'" & TbImport.Rows(i).Item("So_Seri_i").ToString.Trim & "'"
        Next
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value

        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ImportK10", _StrDetail & "#" & _Ngay_Ct.ToString("yyyyMMdd") & "#" & M_Stt_Rec & "#" & M_Ma_CT & "#" & TxtMa_Dvcs.Text & "#" & M_User_Name)
