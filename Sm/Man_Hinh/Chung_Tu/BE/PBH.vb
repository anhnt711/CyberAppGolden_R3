...
#Region "Columns Edit"   
    ...
    Dim EditMa_Kx, EditMa_Mau, Editma_Db_I, EditMa_Xe_I, EditMa_KX_I, EditMa_Tuyen_I, EditMa_Ts, EditMa_Cc As New Cyber.Fill.CyberColumnGridView
    Dim EditSo_Luong, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub PBH_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub L_Ma_TD3_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        'Lấy thông tin từ HDX
        V_GetTTHD()
    End Sub
    Private Sub V_GetTTHD()
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        ...
        Dim _Ma_TD3 As String = Dv_DetailTmp.Item(iRow).Item("Ma_TD3_I")
        Dim _Dt As Date = TxtNgay_Ct.Value

        Dim _DsDetail As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetTTHDPBH", _Ma_TD3 & "#" & _Dt.ToString("yyyyMMdd") & "#" & M_Stt_Rec.Trim & "#" & TxtMa_Dvcs.Text.Trim & "#" & M_User_Name.Trim)

        If _DsDetail.Tables.Count < 1 Then
            _DsDetail.Dispose()
            Exit Sub
        End If
        ...
        'Gán giá trị
        Dv_DetailTmp.Item(iRow).BeginEdit()
        If _DsDetail.Tables(0).Columns.Contains("Ma_HD_I") Then Dv_DetailTmp.Item(iRow).Item("Ma_HD_I") = _DsDetail.Tables(0).Rows(0).Item("Ma_HD_I")        
        If _DsDetail.Tables(0).Columns.Contains("Ten_Kh") Then Dv_DetailTmp.Item(iRow).Item("Ten_HD_I") = _DsDetail.Tables(0).Rows(0).Item("Ten_Kh")
        If _DsDetail.Tables(0).Columns.Contains("Dia_Chi") Then Dv_DetailTmp.Item(iRow).Item("Dia_Chi") = _DsDetail.Tables(0).Rows(0).Item("Dia_Chi")
        If _DsDetail.Tables(0).Columns.Contains("Dien_Thoai") Then Dv_DetailTmp.Item(iRow).Item("Dien_Thoai") = _DsDetail.Tables(0).Rows(0).Item("Dien_Thoai")
        If _DsDetail.Tables(0).Columns.Contains("So_Luong") Then Dv_DetailTmp.Item(iRow).Item("So_Luong") = _DsDetail.Tables(0).Rows(0).Item("So_Luong")
        If _DsDetail.Tables(0).Columns.Contains("Ma_Kx") Then Dv_DetailTmp.Item(iRow).Item("Ma_Kx") = _DsDetail.Tables(0).Rows(0).Item("Ma_Kx")
        If _DsDetail.Tables(0).Columns.Contains("Ten_Kx") Then Dv_DetailTmp.Item(iRow).Item("Ten_Kx") = _DsDetail.Tables(0).Rows(0).Item("Ten_Kx")
        If _DsDetail.Tables(0).Columns.Contains("Ma_Mau") Then Dv_DetailTmp.Item(iRow).Item("Ma_Mau") = _DsDetail.Tables(0).Rows(0).Item("Ma_Mau")
        If _DsDetail.Tables(0).Columns.Contains("Ten_Mau") Then Dv_DetailTmp.Item(iRow).Item("Ten_Mau") = _DsDetail.Tables(0).Rows(0).Item("Ten_Mau")
        If _DsDetail.Tables(0).Columns.Contains("Ngay_Ky") Then Dv_DetailTmp.Item(iRow).Item("Ngay_Ky") = _DsDetail.Tables(0).Rows(0).Item("Ngay_Ky")
        Dv_DetailTmp.Item(iRow).EndEdit()
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_HD_I").ToString.Trim = "" Then
