...
#Region "Columns Edit"
    ...
    Dim EditTen_Kx, EditMa_Mau, EditTen_Mau, EditSo_Khung, EditSo_May, EditTen_Kho_i, EditTen_Tk_Vt, EditTen_Nx_I As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub XTS_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub V_SetSD(ByVal iRow As Integer)
        ...
        If _Return Is Nothing Then Return

        If Dv_DetailTmp.Table.Columns.Contains("Ton13") Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ton13") = If(_Return("SL").ToString = "", 0, _Return("SL"))
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
    End Sub
#End Region
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not CyberSupport.V_ChkExistNovoucher(AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Stt_Rec, M_Ma_CT, TxtSo_Ct.Text.Trim, _Ngay_Ct, _Ngay_LCt, M_Ma_Dvcs, M_User_Name, CyberSmlib) Then Exit Sub '---Kiem tra trung so chung tu
        '----------------------------------------------------------------------------------------------------------------------
        If Not ChkPN7PX7(M_Mode) Then Exit Sub
        '----------------------------------------------------------------------------------------------------------------------
    ...
    Private Function ChkVsave() As Boolean
        ...
        ChkVsave = True
    End Function
    Private Function ChkPN7PX7(ByVal _Mode) As Boolean
        ...
        Next
        DsTmp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkPN7PX7", nCount.ToString.Trim & "#" & _Mode & "#" & _Dt.ToString("yyyyMMdd") & "#" & _So & "#" & strMa_Kx & "#" & strMa_Mau & "#" & strSo_Khung & "#" & strSo_May & "#" & strSo_Po & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_New()
        If Dt_DetailTmp.Rows.Count < 1 Then
            ...
            If Not M_Tk_Co.Trim = "" Then
                DrCurren.Item("Tk_Vt") = M_Tk_Co.Trim
                If DrCurren.Table.Columns.Contains("Ten_Tk_Vt") Then DrCurren.Item("Ten_Tk_Vt") = M_Ten_Tk_Co.Trim
            End If
