...
#Region "Columns Edit"
    ...
    Dim EditTen_Kx, EditTen_Kho_i, EditTen_Nx_I, EditTen_Tk_VT, EditSo_Luong, EditGia_NT, EditGia, EditTien_NT, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub PXX_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub V_SetSD(ByVal iRow As Integer)
        ...
        Dim _Ma_Kho As String = If(Dv_DetailTmp.Table.Columns.Contains("Ma_Kho_I"), Dv_DetailTmp.Item(iRow).Item("Ma_Kho_I").ToString.Trim(), "")
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        nCount = Dt_DetailTmp.Rows.Count
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_Mau").ToString.Trim = "" Then
                Dt_DetailTmp.Rows(iRow).Delete()
                Dt_DetailTmp.AcceptChanges()
            End If
        Next
        nCount = Dt_DetailTmp.Rows.Count
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("So_khung").ToString.Trim = "" Then
                Dt_DetailTmp.Rows(iRow).Delete()
                Dt_DetailTmp.AcceptChanges()
            End If
        Next
        nCount = Dt_DetailTmp.Rows.Count
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("So_May").ToString.Trim = "" Then
                Dt_DetailTmp.Rows(iRow).Delete()
                Dt_DetailTmp.AcceptChanges()
            End If
        Next
