...
#End Region
#Region "Columns Edit"
    Dim EditTk_VT, EditMa_kh_I As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
    ...
    Dim EditTien_NT, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEdiors.Repository.RepositoryItemTextEdit
    Dim EditTen_Tk_VT, EditTen_kh_i As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
#End Region
...
#End Region
    Private Sub PN1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Tinh_Tien()
        ...
        If Not ChkSua_Thue.Checked Then
            nTien_Hang_NT = TxtT_Tien_NT.Double
            nTien_Hang = TxtT_Tien.Double
    ...
    Private Sub L_Tk_VT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Tk_Vt(M_Mode, M_Ma_CT, sender, DetailGRV)
        Dim iRow As Integer = -1
        If DrReturn Is Nothing Then

            iRow = DetailGRV.GetFocusedDataSourceRowIndex
            If iRow < 0 Then Exit Sub
            If Dv_DetailTmp.Item(iRow).Item("Tk_VT").ToString.Trim = "" Then
...
#Region "Phan bo"
    Private Sub V_Phan_Bo_Thue()
        ...
        For iRow As Integer = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            Dt_DetailTmp.Rows(iRow).Item("T_TT_NT") = Dt_DetailTmp.Rows(iRow).Item("Tien_NT") + Dt_DetailTmp.Rows(iRow).Item("Thue_NT")
            Dt_DetailTmp.Rows(iRow).Item("T_TT") = Dt_DetailTmp.Rows(iRow).Item("Tien") + Dt_DetailTmp.Rows(iRow).Item("Thue")
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_Phan_Bo_Thue()
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.ChkVoucherInput(Me, AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Ma_CT, _Ngay_Ct, CbbMa_GD.SelectedValue, M_Stt_Rec, CbbMa_Post.SelectedValue, M_Ma_Dvcs, M_User_Name, Dt_DetailTmp, "Ten_Tk,TEN_KH", "TEN_Tk_VT,TEN_KH_I", CyberSmlib, CyberSupport, CyberSmodb, CyberLoading) Then Exit Sub
        '----------------------------------------------------------------------------------------------------------------------
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Tk_VT").ToString.Trim = "" Or (Dt_DetailTmp.Rows(iRow).Item("Tien") = 0 And Dt_DetailTmp.Rows(iRow).Item("Tien_Nt") = 0) Then
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            Dt_DetailTmp.Rows(iRow).Item("Ma_Thue_I") = TxtMa_Thue.Text
    ...
    Private Sub V_CreateVAT()
        ...
        drMaster.Item("Tk_Thue_No") = TxtTk_Thue_No.Text
    ...
    Private Sub V_New()
        If Dt_DetailTmp.Rows.Count < 1 Then
            ...
            If Not M_Tk_No.Trim = "" Then
                DrCurren.Item("Tk_VT") = M_Tk_No.Trim
                If DrCurren.Table.Columns.Contains("Ten_Tk_VT") Then DrCurren.Item("Ten_Tk_VT") = M_Ten_Tk_No.Trim
            End If
