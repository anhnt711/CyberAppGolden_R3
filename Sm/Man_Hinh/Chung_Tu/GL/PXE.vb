...
#Region "Columns Edit"
    ...
    Dim EditTen_Vt, EditDvt, EditTen_kho_i, EditTen_Nx_I, EditTen_Tk_VT, EditSo_Luong, EditGia_NT, EditGia, EditTien_NT, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
#End Region
...
#Region "Load And Set And Default"
    Dim _Para_RO As String = ""
    Dim M_Ma_Kho_X As String = ""
    Dim M_Ma_Kho_N As String = ""
    Private Sub V_LoadNew(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If _Para_RO.Trim = "" Then Exit Sub
        If Dt_Master.Rows.Count > 0 Then Exit Sub

        'If Not Dt_Master Is Nothing Then Dt_Master.Clear()
        'If Not Dt_Detail Is Nothing Then Dt_Detail.Clear()
        'If Not Dt_DetailTmp Is Nothing Then Dt_DetailTmp.Clear()
        'If Not Dt_DetailVat Is Nothing Then Dt_DetailVat.Clear()
        'If Not Dt_DetailVaTTmp Is Nothing Then Dt_DetailVaTTmp.Clear()

        V_New(sender, e)
        If Not M_Mode = "M" Then Exit Sub
        TxtLenh_RO.Text = _Para_RO
        TxtLenh_RO.Visible = True
        LabLenh_RO.Visible = True
    ...
    Private Sub V_Getdefault()
        ...
        '---------------------------------------------------------------------------------------------'
        M_Ma_Kho_X = M_Para(M_VT_PARA + 4).Trim
        M_Ma_Kho_N = M_Para(M_VT_PARA + 5).Trim

        Try
            _Para_RO = M_Para(M_VT_PARA + 6).Trim
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub L_Lenh_Ro(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        L_Lenh_Ro()
    End Sub
    Private Sub L_Lenh_Ro()
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim Dt As Date = TxtNgay_Ct.Value
        Dim _DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPXEFromRO", TxtLenh_RO.Text + "#" + Dt.ToString("yyyyMMdd") + "#" + M_Stt_Rec.Trim + "#" + M_Ma_CT + "#" + CbbMa_Post.SelectedValue.ToString.Trim + "#" + M_Mode + "#" + TxtMa_Dvcs.Text + "#" + M_User_Name)
        CyberSmodb.SetNotNullTable(_DsTmp.Tables(0))
        If _DsTmp.Tables.Count = 0 Then Return
        If Not CyberSupport.V_MsgChk(_DsTmp.Tables(0), osysvar, M_LAN) Then
            _DsTmp.Dispose()
            Exit Sub
        End If
        '-------------------------------------------------------------------------------
        CyberSmodb.SetValueTObj_1(Me, _DsTmp.Tables(0).Rows(0))
        '-------------------------------------------------------------------------------
        If _DsTmp.Tables.Count = 1 Then Return
        CyberSmodb.SetNotNullTable(_DsTmp.Tables(1))
        Dt_DetailTmp.Clear()
        CyberSmodb.SQLTbToTb(_DsTmp.Tables(1), Dt_DetailTmp)
        _DsTmp.Dispose()
        UpdateList()
    End Sub
...
#Region "Button"
    ...
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_New()
        TxtMa_kho.Text = M_Ma_Kho_X
        TxtMa_khoN.Text = M_Ma_Kho_N

        If Not TxtMa_kho.Text.Trim = "" Then TxtTen_kho.Text = CyberSmodb.SQLGetvalue(AppConn, "Ten_kho", "DmKho", "Ma_kho = N'" + M_Ma_Kho_X.Trim + "'", CyberSmlib) Else TxtTen_kho.Text = ""
        If Not TxtMa_khoN.Text.Trim = "" Then TxtTen_khoN.Text = CyberSmodb.SQLGetvalue(AppConn, "Ten_kho", "DmKho", "Ma_kho = N'" + M_Ma_Kho_N.Trim + "'", CyberSmlib) Else TxtTen_khoN.Text = ""
