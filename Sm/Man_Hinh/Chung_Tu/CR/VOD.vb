...
#Region "Khai bao bien Paramater----------------------"
    ...
    Dim _tbTiep_Nhan, _tbMuc_Do, _tbPhuong_tienTC, _tbHai_Long As DataTable
    Public vTbTiep_nhan, vTbMuc_do, vTbPhuong_tienTC, vtbHai_long As DataView
#End Region
...
#End Region
#Region "Columns Edit"    
    Dim EditMa_Lv, EditTen_LV, EditMa_VD, EditMa_BpHH, EditTen_BPHH, EditMa_HTHH, EditTen_HTHH, EditNgay_TT, EditDien_GiaiTTCT As New Cyber.Fill.CyberColumnGridView
    ...
    Dim EditThac_mac, EditTra_loi, EditMa_LvTM, EditTen_LVTM, EditMa_VDTM, EditTen_VDTM, EditMa_BPXL, EditMa_HsXL As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub VOD_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
        V_GetColumn()
        V_AddHandler()
        V_LoadTTGQ("1")
...
#Region "Load And Set And Default"
    ...
    Private Sub V_LoadTTGQ(ByVal Status As String)
        ...
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_VOD_TTGQFrom", Status + "#" + M_Stt_Rec + "#" + M_Ma_Dvcs + "#" + M_User_Name)
    ...
    Private Sub V_Load()
        ...
        '--GridTTGQ
        Dt_DetailTTGQ = M_DsData.Tables(2)
        ...
        DetailTTGQ.DataSource = Dv_DetailTTGQtmp
        Me.DetailGRVTTGQ.GridControl = Me.DetailTTGQ
        CyberFill.V_FillVoucher(DetailGRVTTGQ, M_LAN, New DataView(M_DsHead.Tables(2)), Dv_DetailTTGQtmp, Me.DrDmct)
        V_SetProperty()
    ...
    Private Sub V_Getdefault()
        ...
        V_LoadDefault()
    End Sub
    Private Sub V_LoadDefaultVOD()
        Dim DsVOC As New DataSet
        DsVOC = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_VOD_GetDefault", M_Ma_CT.Trim & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim())
        ...
        _tbPhuong_tienTC = DsVOC.Tables(2).Copy
        vTbPhuong_tienTC = New DataView(_tbPhuong_tienTC)
        ...
        CyberFill.V_FillComBoxValue(CmbMa_PTLL, vTbPhuong_tienTC, "Ma_PTLL", "Ten_PTLL", "")
        '---------------------------------------------------------------------------------------------'
        ...
        '--------------------------------------------------------------------------------------------
        If _tbPhuong_tienTC.Columns.Contains("Ngam_Dinh") Then
            For i As Integer = 0 To _tbPhuong_tienTC.Rows.Count - 1
                If _tbPhuong_tienTC.Rows(i).Item("Ngam_Dinh").ToString.Trim = "1" Then
                    M_Ma_TN_Ngam_Dinh = _tbPhuong_tienTC.Rows(i).Item("ma_PTLL").ToString.Trim
                    Exit For
                End If
            Next
        End If
    ...
    Private Sub V_GetColumn()
        ...
        EditMa_HTHH.GetColumn(DetailGRV, "Ma_HTHH")
        EditMa_BPXL.GetColumn(DetailGRV, "Ma_BPXL_i")
        EditMa_HsXL.GetColumn(DetailGRV, "Ma_HsXL_i")
    End Sub
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()        
        ...
        EditMa_BPXL.V_ActiLookUpColumn(AddressOf V_Ma_BPXL, AddressOf L_Ma_BPXL)
        EditMa_HsXL.V_ActiLookUpColumn(AddressOf V_Ma_HsXL, AddressOf L_Ma_HsXL)
    End Sub
    ...
    Protected Overrides Sub V_Databing()
        ...
        If Drv Is Nothing Then
            TxtNgay_Ct.Value = Now.Date.AddHours(Now.Hour).AddMinutes(Now.Minute)
...
#Region "Valid - Detail - Core"
    ...
    End Sub
#End Region
#Region "Ma_Lv,Ma_VD,Ma_BPXL,Ma_HsXL"
    Private Sub V_Ma_Lv(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_Lv", "CRMDMLVTM", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    ...
    Private Sub V_Ma_VD(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        '
        CyberSupport.V_Lookup("Ma_VD", "CRMDMVDTM", "Ma_Lv='" + _Ma_Lv + "'", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_VD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        UpdateList()
    End Sub
    Private Sub V_Ma_BPXL(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        '
        CyberSupport.V_Lookup("Ma_Bp", "Dmbp", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_BPXL(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
        ...
        If iRow < 0 Then Exit Sub
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow)("Ma_BPXL_i") = ""
            Exit Sub
        End If
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_BPXL_i") = DrReturn.Item("Ma_Bp").ToString.Trim
        Dv_DetailTmp.Item(iRow).Item("Ten_BpXl_i") = DrReturn.Item("Ten_Bp").ToString.Trim
        Dv_DetailTmp.Item(iRow).EndEdit()

        If Dv_DetailTmp.Item(iRow).Item("Ma_BPXL_i") = "" Then
            Dv_DetailTmp.Item(iRow).Item("Ten_BpXl_i") = ""
        End If

        Dv_DetailTmp.Table.AcceptChanges()
        UpdateList()
    End Sub
    Private Sub V_Ma_HsXL(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
    ...
    End Sub
    Private Sub L_Ma_HsXL(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Lookup(M_Mode, M_Ma_CT, sender, True)
        Dim iRow As Integer = -1
        ...
        If iRow < 0 Then Exit Sub
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow)("Ma_HsXL_i") = ""
            Exit Sub
        End If
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_HsXL_i") = DrReturn.Item("Ma_Hs").ToString.Trim
        Dv_DetailTmp.Item(iRow).Item("Ten_Hsxl_i") = DrReturn.Item("Ten_Hs").ToString.Trim
        Dv_DetailTmp.Item(iRow).EndEdit()

        If Dv_DetailTmp.Item(iRow).Item("Ma_HsXL_i") = "" Then
            Dv_DetailTmp.Item(iRow).Item("Ten_Hsxl_i") = ""
        End If
...
#Region "Button"
    Private Sub V_Save(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        CyberInput.V_Save_Default_TTCP(Dt_DetailTTGQtmp, M_Is_Visible_TTCP, CmbMa_TTCP_H, M_Ma_CT, CyberSmodb)
        '----------------------------------------------------------------------------------------------------------------------
        ...
        If Not CyberSmodb.V_SysCheckSaveVoucher(AppConn, osysvar, {Dt_DetailTmp, CyberSmodb.V_ConvertDrToTb(drMaster), Dt_DetailTTGQtmp}, {M_Ct, M_Ph, "CTVODTTGQ"}, M_Mode, CbbMa_Post.SelectedValue.ToString.Trim, CmbMa_GD.SelectedValue, M_Ma_CT, M_Stt_Rec, M_LAN, TxtMa_Dvcs.Text, M_User_Name, CyberSmlib, CyberSupport, DsSave) Then
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim CP_ChkInPut_CV As String = "CP_SysChkVoucherInput" & M_Ma_CT.Trim & "_RO"
    ...
    Private Sub V_Copy(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Frm.So_CT = ""
        Frm.Ngay_CT = Now.Date.AddHours(Now.Hour).AddMinutes(Now.Minute)
    ...
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        '--------------
        TxtNgay_Gui.Text = "01/01/1900 00:00"
        TxtNgay_GQ.Text = "01/01/1900 00:00"
    ...
    Private Sub V_GetNgamDinh()
        ...
        Try
            CmbMa_PTLL.SelectedValue = M_Ma_TN_Ngam_Dinh
        Catch ex As Exception
        End Try
        Try
...
#Region "Add Or Detete Item"
    ...
    Private Sub V_AddItemTTGQ(Optional ByVal iRow As Integer = -1)
        ...
        CyberSupport.SetCarryOn(DrvOld, Dv_DetailTTGQtmp.Table.Rows(Dv_DetailTTGQtmp.Table.Rows.Count - 1), New DataView(M_DsHead.Tables(2)))
