...
Public Class HDB_Loc
    ...
    Private Sub HDB_Loc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Save_OK = False
        Me.Text = "Loc phieu xuat kho theo lenh"
        V_Load()
        V_GetColumn()
        TxtMa_Kh.Focus()
        '------------------------------------------------------------------------------------
    ...
    Sub V_Loaddatatabase(ByVal _Type As String)
        ...
        Dim _DsTmp As DataSet = CyberInput.V_GetHDBFromPXL(AppConn, CyberSmlib, _Type, IIf(ChkPX.Checked, "1", "0"), _Dt1.ToString("yyyyMMdd"), _Dt2.ToString("yyyyMMdd"), TxtLenh_SO.Text, TxtMa_Kh.Text, M_Stt_rec, M_ma_Ct, M_Ma_Dvcs, M_User_Name)
...
#Region "Grid..."
    ...
    Private Sub V_BilldtingRo(ByVal MasterGRV As DevExpress.XtraGrid.Views.Grid.GridView, ByVal Dv_Master As DataView, ByVal Dv_DetailTmp As DataView)
        ...
        If Not Dv_DetailTmp.Table.Columns.Contains("Stt_rec_PXL") Then Exit Sub
        If Not Dv_Master.Table.Columns.Contains("Stt_rec_PXL") Then Exit Sub
        ...
        Try
            ...
            If (Not String.IsNullOrEmpty(selectedRow("Stt_rec_PXL").ToString)) Then
                filter = "Stt_Rec_PXL='" + selectedRow("Stt_rec_PXL").ToString() + "'"
    ...
    Private Sub V_Detail(ByVal status As Boolean, ByVal SttRec As String)
        ...
        For i = 0 To DtTable2.Rows.Count - 1
            If DtTable2.Rows(i).Item("Stt_rec_PXL").ToString.Trim = SttRec.Trim Then
        ...
        For iRow = 0 To DvTable1.Count - 1
            If DvTable1.Item(iRow).Item("Tag").ToString.Trim.ToUpper = "True".Trim.ToUpper Or DvTable1.Item(iRow).Item("Tag").ToString.Trim = "1" Then
                For i = 0 To DvTable2.Count - 1
                    If (DvTable2.Item(i).Item("Tag").ToString.Trim.ToUpper = "True".Trim.ToUpper Or DvTable2.Item(i).Item("Tag").ToString.Trim = "1") And DvTable2.Item(i).Item("Stt_rec_PXL").ToString.Trim = DvTable1.Item(iRow).Item("Stt_rec_PXL").ToString.Trim Then
        ...
        For i As Integer = 0 To DtTable1.Rows.Count - 1
            If DtTable1.Rows(i).Item("Tag").ToString.Trim.ToUpper = "1" Or DtTable1.Rows(i).Item("Tag").ToString.Trim.ToUpper = "TRUE" Then
                For j As Integer = 0 To DtTable2.Rows.Count - 1
                    If (DtTable2.Rows(j).Item("Tag").ToString.Trim.ToUpper = "1" Or DtTable2.Rows(j).Item("Tag").ToString.Trim.ToUpper = "TRUE") And DtTable2.Rows(j).Item("Stt_Rec_PXL").ToString.Trim = DtTable1.Rows(i).Item("Stt_Rec_PXL").ToString Then
