...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        TxtTong_Cong.InputMask = IIf(DrDmct.Item("M_IP_MK_SL").ToString.Trim <> "", DrDmct.Item("M_IP_MK_SL").ToString.Trim, TxtTong_Cong.InputMask)
    ...
    End Sub
    Private Sub V_GetCa()
        Dim DsCa As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetMa_Ca", TxtMa_Dvcs.Text.Trim & "#" & M_User_Name.ToString.Trim)
        tbKQ = DsCa.Tables(0).Copy
        vtbKQ = New DataView(tbKQ)
        CyberFill.V_FillComBoxValue(CmbCa, vtbKQ, "Ma_Ca", "Ten_Ca", "")
    End Sub
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            nT_Cong = nT_Cong + If(Dt_DetailTmp.Columns.Contains("Tong_Conglam"), Dv_DetailTmp.Item(iRow).Item("Tong_Conglam"), 0)
        Next
        TxtTong_Cong.Double = nT_Cong
        TxtTong_Cong.Refresh()
    End Sub
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Tong_CongLam") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Cong_Chinh") + Dv_DetailTmp.Item(iRow).Item("Cong_Khac") + Dv_DetailTmp.Item(iRow).Item("Them_Gio"), M_R_TIEN)
