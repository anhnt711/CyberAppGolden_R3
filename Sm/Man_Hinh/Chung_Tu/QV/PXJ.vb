...
#Region "Columns Edit"
    ...
    Dim EditMa_Ve, EditMa_Vitri, EditMa_Kho_I, EditMa_Vv_I, EditMa_phi_i, EditMa_HD_I, EditMa_Sp_I, EditMa_ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_HS_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
    Dim Editma_Db_I, EditMa_Xe_I, EditMa_KX_I, EditMa_Tuyen_I, EditMa_Ts, EditMa_Cc, EditSo_Luong, EditSeri_dau, EditSeri_cuoi As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
    Dim EditTen_Ve, EditDvt, EditTen_kho_i, EditTen_Nx_I, EditTen_TK_VT, EditGia_NT, EditGia, EditTien_NT, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_Ve As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
#End Region
...
#End Region
    Private Sub PXE_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditMa_Ve.GetColumn(DetailGRV, "Ma_Ve")
        EditTen_Ve.GetColumn(DetailGRV, "Ten_Ve")
        ...
        EditSeri_dau.GetColumn(DetailGRV, "Seri_dau")
        EditSeri_cuoi.GetColumn(DetailGRV, "Seri_cuoi")
        ...
        EditSo_Ve.GetColumn(DetailGRV, "SO_Ve")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler TxtLenh_Vt.Leave, AddressOf L_Lenh_Ve
    End Sub
    Private Sub V_AddHandler_Detail()
        If Not EditSeri_dau.Column Is Nothing Then AddHandler EditSeri_dau.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditSeri_cuoi.Column Is Nothing Then AddHandler EditSeri_cuoi.EditColumn.Leave, AddressOf L_Tinh_Toan
        ...
        '--Vat tu-kho
        EditMa_Ve.V_ActiLookUpColumn(AddressOf V_Ma_Ve, AddressOf L_Ma_Ve)
        ...
        If Not EditSo_Ve.Column Is Nothing Then AddHandler EditSo_Ve.EditColumn.Leave, AddressOf L_So_Ve
...
#Region "Lenh/SO/Po/Ro/Ve"
    ...
    End Sub
    Private Sub L_Lenh_Ve(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim _Dt As Date = TxtNgay_Ct.Value
...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        '--------------------------------------------------------------------------------
        Dim _Dt As Date = TxtNgay_Ct.Value
        CyberInput.V_Tinh_Sl_Ve(AppConn, Dv_DetailTmp, iRow, _Dt, M_Ma_CT, M_Mode, M_Ma_Dvcs, M_User_Name, CyberSmlib)
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Lo(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_SetSD(iRow)
    End Sub
    Private Sub V_Ma_Ve(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Ma_Ve(M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_Ve(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Ma_Ve(M_Mode, M_Ma_CT, sender, DetailGRV)
        Dim iRow As Integer = -1
        If DrReturn Is Nothing Then
            iRow = DetailGRV.GetFocusedDataSourceRowIndex
            If iRow < 0 Then Exit Sub
            If Dv_DetailTmp.Item(iRow).Item("MA_Ve").ToString.Trim = "" Then
    ...
    Private Sub V_SetSD(ByVal iRow As Integer)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If iRow < 0 Then Exit Sub
        Dim _Ma_Ve As String = Dv_DetailTmp.Item(iRow).Item("Ma_Ve").ToString.Trim
        Dim _Ma_kho As String = TxtMa_Kho.Text '----- Dv_DetailTmp.Item(iRow).Item("Ma_Kho_I").ToString.Trim
        ...
        'Dim _Return As DataRow = CyberSupport.V_SetSD(StatusFooter1, AppConn, "Ve", M_Mode, _Ma_Ve, _Ma_kho, _Ma_Vitri, _Ma_Lo, M_Stt_Rec, M_Ma_CT, M_Ma_Dvcs, M_User_Name, CyberSmlib)
...
#Region "Tai khoan "
    ...
    End Sub
#End Region
#Region "So_So/So_PO/So_RO/So_Ve"
    ...
    End Sub
    Private Sub L_So_Ve(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        CyberInput.V_Save_Default_TTCP(Dt_DetailTmp, M_Is_Visible_TTCP, CbbMa_TTCP_H, M_Ma_CT, CyberSmodb)
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.ChkVoucherInput(Me, AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Ma_CT, _Ngay_Ct, CbbMa_GD.SelectedValue, M_Stt_Rec, CbbMa_Post.SelectedValue, M_Ma_Dvcs, M_User_Name, Dt_DetailTmp, "Ten_Tk,TEN_KH", "TEN_Ve,TEN_KHO_I", CyberSmlib, CyberSupport, CyberSmodb, CyberLoading) Then Exit Sub
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.V_ChkExistNovoucher(AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Stt_Rec, M_Ma_CT, txtSo_ct.Text.Trim, _Ngay_Ct, _Ngay_LCt, M_Ma_Dvcs, M_User_Name, CyberSmlib) Then Exit Sub '---Kiem tra trung so chung tu
        '----------------------------------------------------------------------------------------------------------------------
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_Ve").ToString.Trim = "" Then
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            'Dt_DetailTmp.Rows(iRow).Item("Ma_kho_I") = TxtMa_kho.Text
...
#End Region
    ...
    Private Sub TxtLookup1_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)

    End Sub

    Private Sub TextBox1_TextChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtTen_KhoN.TextChanged

    End Sub

    Private Sub TxtLookup1_EditValueChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtMa_KhoN.EditValueChanged
