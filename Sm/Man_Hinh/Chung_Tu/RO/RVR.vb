...
#Region "Khai bao bien Private"
    ...
    Dim DtPost, DtMaGD, DtMaRa As DataTable

    Dim Dt_DmMucSBD, Dt_DmMucSDS As DataTable


    Dim M_Ma_Ra, M_Tu_Dong, M_Camera As String

#End Region
    Private Sub RVR_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        'CyberFill.V_FillVoucher(DetailGRV, M_LAN, New DataView(M_DsHead.Tables(1)), Dv_Detail, Me.DrDmct)
        ...
        DetailGRV.ViewCaption = "DANH SÁCH XE VÀO/RA CỔNG"
    ...
    Private Sub V_Getdefault()
        ...
        M_Ma_Ra = M_Para(M_VT_PARA + 3).Trim
        M_Tu_Dong = M_Para(M_VT_PARA + 4).Trim
        M_Camera = M_Para(M_VT_PARA + 5).Trim
        '---------------------------------------------------------------------------------------------'
        ...
        DtMaRa = CyberSmodb.OpenTableKey(AppConn, Nothing, "DmRa", "Ma_Ra", "1=1", CyberSmlib)

        Dt_DmMucSBD = CyberSmodb.OpenTableKey(AppConn, Nothing, "DmMucSBD", "Muc_SBD", "1=1", CyberSmlib)
        Dt_DmMucSDS = CyberSmodb.OpenTableKey(AppConn, Nothing, "DmMucSDS", "Muc_SDS", "1=1", CyberSmlib)
        ...
        CyberFill.V_FillComBoxValue(Me.CbbMa_Ra, DtMaRa, "Ma_Ra", If(M_LAN = "V", "Ten_Ra", "Ten_Ra2"), M_Ma_Ra)
    ...
    Private Sub V_SetFocus(ByVal _Loai As String)
        ...
        Select Case _Loai
            Case "M"
                TxtMa_Xe.Focus()
                If M_Tu_Dong = "1" Then ChkTu_Dong.Checked = True Else ChkTu_Dong.Checked = False
                If M_Camera = "1" Then ChkCamera.Checked = True Else ChkCamera.Checked = False
                If M_Camera = "1" Then ChkCamera.Visible = True Else ChkCamera.Visible = False
            Case "L"
                CmdPrint.Select()
        End Select
        ChkCamera.Visible = False
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler ChkTu_Dong.CheckedChanged, AddressOf V_Tu_Dong
        '----------------------------------------------------------------------------------------------------------
        AddHandler ChkSCC.CheckedChanged, AddressOf V_Chk_SSC
        AddHandler ChkSDS.CheckedChanged, AddressOf V_Chk_DS
...
#End Region
    Private Sub V_GetAndSetChkbSCC_DS_ALL(ByVal stautus As String)
        ...
        '    If ChkBSCC.Checked Then
        '        TxtLoai_SC.Text = "1"
        '    End If
        '    If ChkBDS.Checked Then
        '        TxtLoai_SC.Text = "2"
        '    End If
        '    If ChkBSCC.Checked And ChkBDS.Checked Then
        '        TxtLoai_SC.Text = "3"
        '    End If
        '    If Not ChkBSCC.Checked And Not ChkBDS.Checked Then
        '        TxtLoai_SC.Text = "1"
        '    End If
        ...
        '        ChkBSCC.Checked = True
        '        ChkBDS.Checked = True
        '        Exit Sub
        '    End If
        '    If TxtLoai_SC.Text = "2" Then
        '        ChkBDS.Checked = True
        '        ChkBSCC.Checked = False
        '        Exit Sub
        '    End If
        '    ChkBSCC.Checked = True
        '    ChkBDS.Checked = False

        'End If
    End Sub
    Private Sub V_Chk_SSC(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        'If Not ChkBSCC.Checked Then If Not ChkBDS.Checked Then ChkBDS.Checked = True
    End Sub
    Private Sub V_Chk_DS(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        'If Not ChkBDS.Checked Then If Not ChkBSCC.Checked Then ChkBSCC.Checked = True
    End Sub

    Private Sub V_Tu_Dong(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub V_Ma_Xe(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode.Trim = "M" Or M_Mode.Trim = "S") Then Exit Sub
        TxtMa_Xe.Text = CyberVoucher.V_FormatBien_So(TxtMa_Xe.Text, IIf(ChkIs_khung.Checked, False, True))

        Dim _Is_Khung As String = IIf(ChkIs_khung.Checked, "1", "0")
        Dim _Is_Kh As String = IIf(ChkIs_KH.Checked, "1", "0")
        Dim _ma_Gd As String = CbbMa_GD.SelectedValue.ToString.Trim
        Dim _ma_Ra As String = CbbMa_Ra.SelectedValue.ToString.Trim
        Dim _ma_Post As String = CbbMa_Post.SelectedValue.ToString.Trim

        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RVRGetKh", _Is_Khung & "#" & _Is_Kh & "#" & _
                                                                                                TxtMa_Xe.Text.Trim & "#" & _
                                                                  _ma_Gd & "#" & _ma_Ra & "#" & M_Mode.Trim & "#" & _ma_Post & "#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        If DsTmp.Tables(0).Columns.Contains("Ten_kh") And TxtTen_kh.Text.Trim = "" Then TxtTen_kh.Text = DsTmp.Tables(0).Rows(0).Item("ten_kh").ToString.Trim
        ...
        If ChkTu_Dong.Checked Then
            If DsTmp.Tables(0).Columns.Contains("Ma_Ra") Then CbbMa_Ra.SelectedValue = DsTmp.Tables(0).Rows(0).Item("Ma_Ra").ToString.Trim
            If DsTmp.Tables(0).Columns.Contains("Ma_GD") Then CbbMa_GD.SelectedValue = DsTmp.Tables(0).Rows(0).Item("Ma_GD").ToString.Trim
        End If
        'If CbbMa_GD.SelectedValue.ToString.Trim = "1" Then
        ChkHen.Checked = False
        Chkfirst_visit.Checked = False
        ChkIs_3D.Checked = False

        If DsTmp.Tables(0).Columns.Contains("Hen") Then If DsTmp.Tables(0).Rows(0).Item("Hen").ToString.Trim = "1" Then ChkHen.Checked = True Else ChkHen.Checked = False
        If DsTmp.Tables(0).Columns.Contains("first_visit") Then If DsTmp.Tables(0).Rows(0).Item("first_visit").ToString.Trim = "1" Then Chkfirst_visit.Checked = True Else Chkfirst_visit.Checked = False
        If DsTmp.Tables(0).Columns.Contains("Is_3D") Then If DsTmp.Tables(0).Rows(0).Item("Is_3D").ToString.Trim = "1" Then ChkIs_3D.Checked = True Else Chkfirst_visit.Checked = False
        ...
        Exit Sub
    End Sub
    Private Sub V_VisibleRa()
        If CbbMa_GD.SelectedValue.ToString.Trim = "2" Then
            LabelMa_ra.Visible = True
            CbbMa_Ra.Visible = True

            ChkIs_KH.Visible = False
            ChkSCC.Visible = False
            ChkSBD.Visible = False
            ChkSDS.Visible = False
            ChkHen.Visible = False
            Chkfirst_visit.Visible = False

        Else
            LabelMa_ra.Visible = False
            CbbMa_Ra.Visible = False
            ...
            ChkHen.Visible = True
            Chkfirst_visit.Visible = True
        End If
        ChkHen.Enabled = False
        Chkfirst_visit.Enabled = False
    End Sub
#End Region
#Region "Valid - Detail"
...
#Region "Button"
    ...
    Private Function ChkSaveDelete(ByVal _Mode As String) As Boolean
        ...
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkSaveDeleterRVR", strField & "#" & strValue & "#" & _
                                                                  _Ngay_Ct.ToString("yyyyMMdd") & "#" & M_Ma_CT & "#" & _
                                                                  M_Stt_Rec & "#" & _Ma_Post & "#" & _Ma_GD & "#" & _Ma_Ra & "#" & _
        ...
        If _Mode.Trim = "M" And _Return And Dstmp.Tables(0).Columns.Contains("Ma_Ra") Then CbbMa_Ra.SelectedValue = Dstmp.Tables(0).Rows(0).Item("Ma_Ra").ToString.Trim
...
#Region "Sms - Import"
    ...
    Private Sub SetTxt()
        ...
        ChkHen.Enabled = False
        Chkfirst_visit.Enabled = False
        ChkIs_3D.Enabled = False

    End Sub
#End Region
#Region "Mức sửa chữa đồng sơn/Mức bảo dưỡng"
