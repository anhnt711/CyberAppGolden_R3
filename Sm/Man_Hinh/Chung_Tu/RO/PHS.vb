...
#End Region
#Region "Columns Edit"
    Dim EditTG_BD, EditTG_KT, EditNgay_BD, EditNgay_KT, EditMa_Cv, EditTen_Hs_I, EditTG, EditTL_CV, _
         EditMa_Vv_I, EditMa_phi_i, EditMa_HD_I, EditMa_Sp_I, EditMa_ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_HS_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
    Dim EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
#End Region
...
#End Region
    Private Sub PHS_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditTG_BD.GetColumn(DetailGRV, "TG_BD")
        EditTG_KT.GetColumn(DetailGRV, "TG_KT")
        EditNgay_BD.GetColumn(DetailGRV, "Ngay_BD")
        EditNgay_KT.GetColumn(DetailGRV, "Ngay_KT")

        EditMa_Cv.GetColumn(DetailGRV, "Ma_Cv")
        EditTen_Hs_I.GetColumn(DetailGRV, "Ten_Hs_I")

        EditTG.GetColumn(DetailGRV, "TG")
        EditTL_CV.GetColumn(DetailGRV, "TL_CV")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        If Not EditTG_BD.Column Is Nothing Then AddHandler EditTG_BD.EditColumn.Leave, AddressOf L_TG_BD
        If Not EditTG_KT.Column Is Nothing Then AddHandler EditTG_KT.EditColumn.Leave, AddressOf L_TG_KT
        If Not EditNgay_BD.Column Is Nothing Then AddHandler EditNgay_BD.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditNgay_KT.Column Is Nothing Then AddHandler EditNgay_KT.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTG.Column Is Nothing Then AddHandler EditTG.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTL_CV.Column Is Nothing Then AddHandler EditTL_CV.EditColumn.Leave, AddressOf L_Tinh_Toan
...
#Region "Valid - Detail - Core"
    ...
    End Sub
    Private Sub L_TG_BD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        If Dv_DetailTmp.Item(iRow).Item("Tg_Bd").ToString.Trim <> "" Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tg_Bd") = CyberVoucher.V_GetTG(Dv_DetailTmp.Item(iRow).Item("Tg_Bd"), 0)
        ...
        V_Tinh_Toan(iRow)
    End Sub
    Private Sub L_TG_KT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        If Dv_DetailTmp.Item(iRow).Item("Tg_KT").ToString.Trim <> "" Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tg_KT") = CyberVoucher.V_GetTG(Dv_DetailTmp.Item(iRow).Item("Tg_KT"), 0)
            Dv_DetailTmp.Item(iRow).EndEdit()
            Dv_DetailTmp.Table.AcceptChanges()
        End If
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        '--------------------------------------------------------------------------------
        If iRow < 0 Then Exit Sub
        Dim _Tg_Bd As String = Dv_DetailTmp.Item(iRow).Item("Tg_Bd")
        Dim _Tg_Kt As String = Dv_DetailTmp.Item(iRow).Item("Tg_Kt")
        Dim _Ngay_Bd As Date = Dv_DetailTmp.Item(iRow).Item("Ngay_Bd")
        Dim _Ngay_Kt As Date = Dv_DetailTmp.Item(iRow).Item("Ngay_Kt")

        Dim _Tg As Decimal = CyberVoucher.V_TinhSoPhut(_Tg_Bd, _Ngay_Bd, _Tg_Kt, _Ngay_Kt)
        If _Tg <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tg") = _Tg
            Dv_DetailTmp.Item(iRow).EndEdit()
            Dv_DetailTmp.Table.AcceptChanges()
        End If
        CyberSupport.V_UpdateGRV(DetailGRV)
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("So_RO").ToString.Trim = "" Or Dt_DetailTmp.Rows(iRow).Item("ma_Hs_I").ToString.Trim = "" Then
