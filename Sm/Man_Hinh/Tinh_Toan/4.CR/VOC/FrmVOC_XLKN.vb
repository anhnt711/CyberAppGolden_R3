...
Public Class FrmVOC_XLKN
    ...
    Dim mRec_Detail As String = "", mRec0_Detail As String = "", mMa_Lv As String = ""
...
#Region "Data"
    Private Sub V_LoadDatabase(ByVal Status As String)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_VOC_XLKN_Load", Status & "#" & Dt1.ToString("yyyyMMdd HH:mm").Trim & "#" & Dt2.ToString("yyyyMMdd HH:mm").Trim & "#" & M_Ma_Dvcs & "#" & M_User_Name.Trim)
    ...
    Private Sub V_BilldtingRo(ByVal MasterGRV As DevExpress.XtraGrid.Views.Grid.GridView, ByVal Dv_Master As DataView, ByVal Dv_DetailTmp As DataView)
        ...
        If Not Dv_DetailTmp.Table.Columns.Contains("Stt_Rec0VOC") Then Exit Sub
        ...
        Try
            ...
            If (Not String.IsNullOrEmpty(selectedRow("Stt_Rec").ToString)) Then
                filter = "Stt_Rec='" + selectedRow("Stt_Rec").ToString() + "' And Stt_Rec0VOC='" + selectedRow("Stt_Rec0").ToString() + "'"
                mRec_Detail = selectedRow("Stt_Rec").ToString()
                mRec0_Detail = selectedRow("Stt_Rec0").ToString()
                mMa_Lv = selectedRow("Ma_Lv").ToString()
    ...
    Private Sub V_AddItem(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim xStt_Rec As String = "", xStt_Rec0 As String = "", xStt_Rec0VOC As String = "", Ma_Lv As String = ""
        Dim iRow As Integer = DetailGRV_Detail.FocusedRowHandle
        If iRow < 0 Then
            xStt_Rec = mRec_Detail
            xStt_Rec0VOC = mRec0_Detail
            Ma_Lv = mMa_Lv
        Else
            Dim selectedRow As DataRowView = Nothing
            Try
                ...
                If (Not String.IsNullOrEmpty(selectedRow("Stt_Rec").ToString)) Then
                    ...
                    xStt_Rec0VOC = selectedRow("Stt_Rec0VOC").ToString()
                    Ma_Lv = selectedRow("Ma_Lv").ToString()
                End If
            Catch ex As Exception
            End Try
        End If

        If xStt_Rec = "" Or xStt_Rec0VOC = "" Then Exit Sub

        Dim Form As New FrmVOC_XLKN_F4
        Form.Mode = "M"
        Form.Para = Me.Para
        ...
        Form.Stt_rec0VOC = xStt_Rec0VOC
        Form.Stt_rec0 = xStt_Rec0
        Form.Ma_Lv = Ma_Lv
        Form.ShowDialog()
        If Form.Save_OK = True Then
            dtDetail.ImportRow(Form.Row)
            vdtDetail.RowFilter = " Stt_Rec ='" & Form.Stt_rec & "' AND Stt_Rec0VOC ='" & Form.Stt_rec0VOC & "'"
        End If
    End Sub
    Private Sub V_EditItem(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim xStt_Rec As String = "", xStt_Rec0VOC As String = "", xStt_Rec0 As String = "", Ma_Lv As String = ""
        Dim iRow As Integer = DetailGRV_Detail.FocusedRowHandle
        ...
        If Form.Save_OK = True Then
            For i As Integer = 0 To dtDetail.Rows.Count - 1
                If dtDetail.Rows(i).Item("Stt_Rec") = Form.Stt_rec And dtDetail.Rows(i).Item("Stt_Rec0") = Form.Stt_rec0 And dtDetail.Rows(i).Item("Stt_Rec0VOC") = Form.Stt_rec0VOC Then
    ...
    Private Sub V_Delete(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim iRow As Integer = DetailGRV_Detail.FocusedRowHandle
        If iRow < 0 Then Exit Sub
        Dim xStt_Rec As String = "", xStt_Rec0VOC As String = "", xStt_Rec0 As String = ""
        Dim selectedRow As DataRowView = Nothing
        Try
            selectedRow = DetailGRV_Detail.GetFocusedRow
            If (Not String.IsNullOrEmpty(selectedRow("Stt_Rec").ToString)) Then
        ...
        Dim DsApproval As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_VOC_XLKN_Del", xStt_Rec & "#" & xStt_Rec0.Trim & "#" & xStt_Rec0VOC.Trim & "#" & M_Ma_Dvcs.Trim & "#" & M_User_Name)
        ...
        For i As Integer = 0 To dtDetail.Rows.Count - 1
            If dtDetail.Rows(i).Item("Stt_Rec") = xStt_Rec And dtDetail.Rows(i).Item("Stt_Rec0VOC") = xStt_Rec0VOC And dtDetail.Rows(i).Item("Stt_Rec0") = xStt_Rec0 Then
                dtDetail.Rows(i).Delete()
                dtDetail.AcceptChanges()
                vdtDetail.RowFilter = " Stt_Rec ='" & xStt_Rec & "' and Stt_Rec0VOC ='" & xStt_Rec0VOC & "'"
                Exit For
            End If
        Next
    ...
    Private Sub V_ChangeUser(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim Form As New FrmVOC_XLKN_User
        Form.Mode = ""
        Form.Para = Me.Para
        Form.Sysvar = Me.Sysvar
        Form.M_LAN = Me.M_LAN
