...
Public Class GLCTPK4
    ...
    Private Sub FGLCTPK4_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        DetailGRV_PopupMenuShowing(DetailGRV, Nothing)
...
#Region "Load data"
    Private Sub V_Load()
        ...
        Dt_Dmct = DsDataLoc.Tables(2).Copy
        Dt_head = DsDataLoc.Tables(3).Copy
        Dt_headDetail = DsDataLoc.Tables(4).Copy
...
#Region "V_AddHandler"
    Private Sub V_AddHandler_Master()
        ...
        RemoveHandler DetailGRV.PopupMenuShowing, AddressOf DetailGRV_PopupMenuShowing
    ...
    Private Sub V_CreateBT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _DsChk As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkPK4", "D" & "#" & _thang1 & "#" & _thang2 & "#" & _Nam & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        For i = 0 To Dt_Master.Rows.Count - 1
            ...
            _DsTmp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GLCTPK4", Dt_Master.Rows(i).Item("Stt_rec").ToString.Trim & "#" & _thang1 & "#" & _thang2 & "#" & _Nam & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        Next
    ...
    Private Sub V_DeleteBT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        For i = 0 To Dt_Master.Rows.Count - 1
            If Not Dt_Master.Rows(i).Item("Tag") Then Continue For
            _DsTmp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GLCTPK4DELETE", "Y" & "#" & Dt_Master.Rows(i).Item("Stt_rec").ToString.Trim & "#" & _thang1 & "#" & _thang2 & "#" & _Nam & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        Next
        If Not _DsTmp Is Nothing Then If _DsTmp.Tables.Count > 0 Then If _DsTmp.Tables(0).Columns.Contains("status") Then CyberSupport.V_MsgChk(_DsTmp.Tables(0), Me.Sysvar, Me.Lan)
...
#Region "Dieu chinh So_Ct/HeSo"
    Private Sub V_F3(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim FrmF3 As GLCTPK4_F3
        FrmF3 = New GLCTPK4_F3
        FrmF3.Lan = Me.Lan
        FrmF3.Para = Me.Para
        FrmF3.Sysvar = Me.Sysvar
        FrmF3.AppConn = Me.AppConn
        ...
        MasterGRV.SelectRow(DetailGRV.FocusedRowHandle)
    End Sub
    Private Sub V_F31(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        Dim _Drv As DataRowView = Dv_DetailTmp.Item(iRow)

        Dim FrmF3 As GLCTPK4_F31
        FrmF3 = New GLCTPK4_F31
        FrmF3.Lan = Me.Lan
        FrmF3.Para = Me.Para
        FrmF3.Sysvar = Me.Sysvar
        FrmF3.AppConn = Me.AppConn
        ...
        Dim rowView As DataRowView
        Dim iCol As Integer = -1
        rowView = V_UpdateItem(Dv_DetailTmp, iRow, _Drv)
        DetailGRV.FocusedRowHandle = 0
        iCol = FindRowHandleByRowObject(DetailGRV, rowView)
        DetailGRV.FocusedRowHandle = iCol
        DetailGRV.ClearSelection()
        DetailGRV.SelectRow(DetailGRV.FocusedRowHandle)
...
#Region "Event"
    ...
    Private Sub MasterGRV_PopupMenuShowing(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs)
        ...
        PopupMenuMasterGrid.ItemLinks.Add(New Cyber.SmLib.CyberMenuPopup(sender, rowHandle, IIf(Lan = "V", "Xóa  " & M_Bar, "Delete " & M_Bar2), AddressOf V_DeleteBT, Shortcut.F8, My.Resources.Xoa_BT_16x16, True, True))
        PopupMenuMasterGrid.ItemLinks.Add(New Cyber.SmLib.CyberMenuPopup(sender, rowHandle, IIf(Lan = "V", "Nhập số chứng từ " & M_Bar, "Enter no " & M_Bar2), AddressOf V_F3, Shortcut.F3, My.Resources.edit_16x16, True, True), True)
    ...
    Private Sub DetailGRV_PopupMenuShowing(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs)
        ...
        Else
            rowHandle = e.HitInfo.RowHandle
        End If
        PopupMenuDetailGrid.ItemLinks.Clear()
        PopupMenuDetailGrid.ItemLinks.Add(New Cyber.SmLib.CyberMenuPopup(sender, rowHandle, IIf(Lan = "V", "Nhập hệ số " & M_Bar, "Enter Coefficient " & M_Bar2), AddressOf V_F31, Shortcut.F3, My.Resources.edit_16x16, True, True))
        '--- End Tuy chọn
        Dim MenuExits As New Cyber.SmLib.CyberMenuPopup(sender, rowHandle, IIf(Lan = "V", "Quay ra", "Exit"), AddressOf V_Exits, My.Resources.cancel_16x16, True, True)
        PopupMenuDetailGrid.ItemLinks.Add(MenuExits, True)
        'PopupMenuMasterGrid.ShowPopup(Control.MousePosition)
        If Not e Is Nothing Then PopupMenuDetailGrid.ShowPopup(Control.MousePosition)
        'End If
    End Sub
#End Region
End Class
