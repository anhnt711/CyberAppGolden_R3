Public Class RO_TT02
    Dim CyberMe As New Cyber.ReportsCustom.Sys
    Dim _Dt_post1, _Dt_post2, _DtMa_TT, _DtGroup, _DtGroupct As DataTable
...
#End Region
    Private Sub V_Load()
        ...
        '--------------------------------------------------------------------------------------------------------------------------------------
        Dim _DsGroup As DataSet = CyberSmLib.SQLExcuteStoreProcedure(AppConn, "CP_SysGetTHRO", "HDS" + "#" + "" + "#" + "" + "#" + Me.Ma_Dvcs + "#" + Me.User_Name)
        _DtGroup = _DsGroup.Tables(0).Copy
        _DtGroupct = _DsGroup.Tables(1).Copy
        Dim _nCount As Integer = _DtGroup.Rows.Count - 1
        For i As Integer = _nCount To 0 Step -1
            If _DtGroup.Rows(i).Item("ma").ToString = "MA_TP" Then
                _DtGroup.Rows(i).Delete()
                Exit For
            End If

        Next
        _DtGroup.AcceptChanges()
        _nCount = _DtGroupct.Rows.Count - 1
        For i As Integer = _nCount To 0 Step -1
            If _DtGroupct.Rows(i).Item("ma").ToString = "MA_TP" Then
                _DtGroupct.Rows(i).Delete()
                Exit For
            End If

        Next
        _DtGroupct.AcceptChanges()
