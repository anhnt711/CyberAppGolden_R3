...
#Region "Load..."
    ...
    Private Sub V_AddHander()
        ...
        AddHandler TxtNgay.Leave, AddressOf V_Ngay_Den
...
#Region "K/xGoi sua chua"
    ...
    Private Sub V_Tinh_Tien_TG()
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_WorkDLH_CPSC", _So_Phut.ToString.Trim.Replace(" ", "").Replace(",", ".") & "#" & M_Loai_SC & "#" &
                                                                  M_Mode & "#" & M_Stt_Rec & "#" &
                                                                  _Ma_Xe & "#" & _Ma_Sc & "#" &
                                                                  _SCC & "#" & _SBD & "#" & _SDS & "#" & _Muc_SBD & "#" & _Muc_SDS & "#" &
...
#Region "Thoi gian/Ngay"
    Private Sub V_Ngay_Den(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim gio As Integer = Integer.Parse(TxtGio_Den.Text.Split(":"c)(0))
        Dim phut As Integer = Integer.Parse(TxtGio_Den.Text.Split(":"c)(1))
        Dim _Ngay_Den As DateTime = Date.Parse(TxtNgay.Text.Split(" "c)(0)) '
        TxtNgay_Den.Value = CyberWork.V_AddTime(_Ngay_Den, gio * 60 + phut, AppConn, CyberSmlib, M_Ma_Dvcs)

        TxtNgay_BD.Value = CyberWork.V_AddTime(TxtNgay_Den.Value, 15, AppConn, CyberSmlib, M_Ma_Dvcs)
        Dim _Ngay_BD As Date = TxtNgay_BD.Value
...
#Region "Hỗ trợ: Gợi ý CVDV/Khoang: Lịch sử sửa chữa, Lịch sử hẹn, Năng lực xưởng "
    ...
    Private Function V_Goi_Y_khoang(ByVal _Strkhoang As String) As DataRow
        ...
        Dim Dstmp As DataSet = CyberWork.V_Goi_Y_khoangDatHen(_Strkhoang, "DLH", M_Stt_Rec, _Is_One_khoang, _Ma_Xe, _So_Ro, _So_Phut, _Ngay_BD,
                                                                         _Loai_Sc, _SCC, _SBD, _SDS, _Muc_SBD, _Muc_SDS,
    ...
    Private Sub V_Goi_Y_CVDV(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _Ngay_Den As Date = TxtNgay.Value
        Dim _Ngay_BD As Date = TxtNgay_BD.Value
        Dim _Ngay_KT As Date = TxtNgay_KT.Value
