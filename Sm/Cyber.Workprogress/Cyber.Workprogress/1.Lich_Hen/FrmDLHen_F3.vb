...
#Region "AddProperty"
    ...
    Public Property Mode() As String
        ...
        End Set
    End Property
    Dim M_Tao_Lai As String = "N"
    Public Property Tao_Lai() As String
        Get
            Tao_Lai = M_Tao_Lai
        End Get
        Set(ByVal Value As String)
...
#Region "Load..."
    Private Sub V_Load(ByVal _Load As String)
        ...
        'If TxtGio_Den.Text = "" Then TxtGio_Den.Text = CyberVoucher.V_GetTG(Strings.Right("00" + Now.Hour.ToString, 2) & Strings.Right("00" + Now.Minute.ToString.Trim, 2), 0)
        Dim _NgayHen As DateTime = TxtNgay_Den.Value
        TxtNgay_Hen.Value = _NgayHen.Date
        TxtGio_Den.Text = _NgayHen.ToString("HH:mm")
        'If TxtNgay_LL_Lan1.Value = Date.MinValue Then TxtNgay_LL_Lan1.Value = Now.Date()
    End Sub
    ...
    Private Sub V_AddHander()
        ...
        AddHandler TxtNgay_Hen.TextChanged, AddressOf V_Ngay_Hen
        AddHandler TxtGio_Den.Leave, AddressOf V_Gio_Den
        ...
        AddHandler ButtMoi.Click, AddressOf V_New
        '-------------------------------------------------------------------------------------------------------
...
#Region "K/xGoi sua chua"
    ...
    Private Sub V_Tinh_Tien_TG()
        ...
        Dim _Ngay_BD As Date '= TxtNgay_BD.Value.ToString)
        DateTime.TryParse(TxtNgay_BD.Text, _Ngay_BD)
        'TxtNgay_KT.Value = _Ngay_BD.AddMinutes(TxtTG_SC.Double)
        If TxtTG_SC.Double <= 0 Then TxtTG_SC.Double = 0
        TxtNgay_KT.Value = CyberWork.V_AddTime(TxtNgay_BD.Value, TxtTG_SC.Double, AppConn, CyberSmlib, M_Ma_Dvcs)
        DsTmp.Dispose()
    End Sub
#End Region
#Region "Chk Loai SC"
...
#Region "Thoi gian/Ngay"
    ...
    Private Sub V_Ngay_Hen(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim _Ngay_Hen As Date = TxtNgay_Hen.Value
        _Ngay_Hen = _Ngay_Hen.Date
        Dim parts() As String = TxtGio_Den.Text.Split(":"c)
        Dim hours As Integer = Convert.ToInt32(parts(0))
        Dim minutes As Integer = Convert.ToInt32(parts(1))

        ' Cộng vào ngày
        TxtNgay_Den.Value = _Ngay_Hen.AddHours(hours).AddMinutes(minutes)
        _Ngay_Hen = TxtNgay_Den.Value
        TxtNgay_BD.Value = CyberWork.V_AddTime(_Ngay_Hen, 15, AppConn, CyberSmlib, M_Ma_Dvcs)
        Dim _Ngay_BD As Date = TxtNgay_BD.Value
        If TxtTG_SC.Double > 0 Then
            TxtNgay_KT.Value = CyberWork.V_AddTime(_Ngay_BD, TxtTG_SC.Double, AppConn, CyberSmlib, M_Ma_Dvcs)
        End If
        V_Ngay_Gx(sender, e)
    End Sub
    Private Sub V_Gio_Den(ByVal sender As System.Object, ByVal e As System.EventArgs)
        TxtGio_Den.Text = CyberVoucher.V_GetTG(TxtGio_Den.Text, 0)
        Dim _Ngay_Hen As Date = TxtNgay_Hen.Value
        _Ngay_Hen = _Ngay_Hen.Date
        Dim parts() As String = TxtGio_Den.Text.Split(":"c)
        Dim hours As Integer = Convert.ToInt32(parts(0))
...
#Region "Save/Quit..."
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_WorkGet_Stt_Rec", "DmDlHen_CVDV#DLH##")
        M_Stt_Rec = DsTmp.Tables(0).Rows.Item(0)("Stt_Rec")
        M_Mode = "M"
        For Each ctrl As Control In Me.Controls
            ...
            End If
        Next
        TxtGio_Den.Text = CyberVoucher.V_GetTG(Strings.Right("00" + Now.Hour.ToString, 2) & Strings.Right("00" + Now.Minute.ToString.Trim, 2), 0)
        TxtMa_Xe.Text = ""
        TxtMa_Kx.Text = ""
        TxtSo_KM.Text = 0
        TxtE_Mail.Text = ""
        ...
        ButtMoi.Enabled = False
        ButtOK.Enabled = True
        M_Ngay_BD = TxtNgay_BD.Value
        M_Ngay_KT = TxtNgay_KT.Value
        V_Load("1")
        V_Tinh_Tien_TG()
        V_TG_SC_TT()
        TxtMa_Xe.Focus()
    ...
    Private Sub V_Nhan(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Else
            ...
            DsTmp.Dispose()
            For Each ctrl As Control In Me.Controls
                If ctrl.Name = "ButtOK" Or TypeOf ctrl Is ComboBox Or TypeOf ctrl Is CheckBox Then
                    ctrl.Enabled = False
                End If
                If ctrl.Name = "ButtMoi" Then
                    ctrl.Enabled = True
                End If
            Next
            'Me.Close()
